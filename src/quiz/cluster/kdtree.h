...
struct KdTree
{
    ...
    void insertNodeHelper (Node *&node,unsigned int depth ,const std::vector<float>& point, int id)
	{
		if(node == NULL)
		{
		...
		{
			unsigned int cd = depth % node->point.size();
				...
				const float distanceTol, Node* node, int depth) 
	{
		if (node != NULL) 
		{
            ...
            unsigned int cd = depth % target.size();
